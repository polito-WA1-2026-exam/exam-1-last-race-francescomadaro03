...
export const validateSubmittedRoute = (graph, segments, assignedStartId, assignedEndId) => {
    ...
    if (segments[0].startId !== assignedStartId) {
        return { isValid: false, error: "The route does not start at the assigned starting station." };
    }
    if (segments[segments.length - 1].endId !== assignedEndId) {
        return { isValid: false, error: "The route does not end at the assigned destination station." };
    }

    const usedSegments = new Set();
    let currentStationId = assignedStartId;
